...
pub fn main() {
    let m = Command::new("rustls benchmarking program")
        .about("Runs rustls benchmarks")
        ...
        .subcommand(
            Command::new("bulk")
                .about("Runs bulk data benchmarks")
                .arg(Arg::new("cipher-suite").required(true))
                .arg(
                    Arg::new("plaintext-size")
                        .value_parser(value_parser!(u64))
                        .default_value("1048576")
                        .required(false),
                )
                .arg(
                    Arg::new("max-fragment-size")
        ...
        .subcommand(
            Command::new("handshake")
                .about("Runs full handshake speed benchmarks")
                .arg(Arg::new("cipher-suite").required(true)),
        )
        .subcommand(
            Command::new("handshake-resume")
                .about("Runs stateful resumed handshake speed benchmarks")
                .arg(Arg::new("cipher-suite").required(true)),
        )
        .subcommand(
            Command::new("handshake-ticket")
                .about("Runs stateless resumed handshake speed benchmarks")
                .arg(Arg::new("cipher-suite").required(true)),
        )
        .subcommand(
            Command::new("memory")
                ...
                .long_about(
                    "This creates `count` connections in parallel (count / 2 clients connected\n\
        ...
        .subcommand(
            Command::new("list-suites")
                .about("Lists the supported values for cipher-suite options")
        )
        .get_matches();

    let options = Options {
        work_multiplier: *m.get_one("multiplier").unwrap(),
    };

    match m.subcommand() {
        Some(("bulk", subm)) => {
            let suite = subm
                .get_one::<String>("cipher-suite")
                .unwrap();
            let len = *subm.get_one("plaintext-size").unwrap();
            let mfs = subm
                .get_one("max-fragment-size")
                .cloned();

            for param in lookup_matching_benches(suite).iter() {
                bench_bulk(param, &options, len, mfs);
            }
        }
        Some((cmd @ "handshake", subm))
        | Some((cmd @ "handshake-resume", subm))
        | Some((cmd @ "handshake-ticket", subm)) => {
            let resume = ResumptionParam::from_subcommand(cmd);
            let suite = subm
                .get_one::<String>("cipher-suite")
                .unwrap();
        ...
        Some(("memory", subm)) => {
            let suite = subm
                .get_one::<String>("cipher-suite")
                .unwrap();
            let count = *subm.get_one("count").unwrap();
...
impl ResumptionParam {
    fn from_subcommand(s: &str) -> Self {
        match s {
            "handshake" => Self::No,
            "handshake-resume" => Self::SessionId,
            "handshake-ticket" => Self::Tickets,
            _ => todo!("unhandled subcommand {s:?}"),
        }
    }
